...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( nanmidrange.length, 1, 'returns expected value' );
	t.end();
});
...
tape( 'the function calculates the mid-range of an array (accessors)', function test( t ) {
	...
	x = [ 1.0, -2.0, -4.0, 5.0, 0.0, 3.0, NaN, NaN ];
	v = nanmidrange( toAccessorArray( x ) );
	t.strictEqual( v, 0.5, 'returns expected value' );
...
tape( 'if provided an array containing a single element, the function returns the element value (accessors)', function test( t ) {
	var v = nanmidrange( toAccessorArray( [ 1.0 ] ) );
	t.strictEqual( v, 1.0, 'returns expected value' );
	t.end();
});
